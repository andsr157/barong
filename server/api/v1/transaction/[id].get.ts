...
export default defineEventHandler(async (event) => {
    ...
    try {
        ...
        if (AuthorizationCheck(session, id).status !== 200) {
            return AuthorizationCheck(session, id);
        }
        ...
        const formattedTransactions = {
            ...
            servicePrice: transactions[0].total * 10 / 100,
            finalTotalPrice: transactions[0].total - (transactions[0].total * 10 / 100),
            status: status,
            review: {
