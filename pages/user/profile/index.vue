...
<template>
  <Header title="Profil" />
  <div class="h-[600px]">
    <div class="h-[238px] bg-brg-primary rounded-br-3xl rounded-bl-3xl flex flex-col justify-center items-center">
      <div class="w-[98px] h-[98px] rounded-full border-[3px] overflow-y-hidden">
        <NuxtImg src="/assets/dummy-profile-user.jpeg" width="98" height="98" class="object-cover w-full h-full" />
      </div>
      <h1 class="font-semibold text-white text-sm">{{ user.user.name }}</h1>
      <h1 class="text-[11px] text-white font-light">{{ user.user.telp }}</h1>
    ...
    <div class="px-6 mt-[34px] flex flex-col gap-[15px]">
      ...
      <NuxtLink class="flex items-center gap-2" to="/user/profile/address">
        <Icon name="uil:map-marker-edit" class="w-5 h-5 text-brg-primary-dark" />
    ...
    <div class="px-6 mt-[34px] flex flex-col gap-[15px]">
      <NuxtLink class="flex items-center gap-2" to="/term_n_condition">
        <Icon name="charm:notes-tick" class="w-5 h-5 text-brg-primary-dark" />
        <h1 class="font-semibold text-sm text-brg-primary-dark">
